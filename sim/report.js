...
export function formatMarkdown(report) {
  const lines = [
    ...
    `- Schedule method: complete ${report.metadata.schedule.cycleSize}-game cycles; ${report.metadata.schedule.cyclicAgentOffsetsPerPlayerCount} cyclic agent offsets at each of ${report.metadata.schedule.playerCounts.join(', ')} players; identical lineup and seat exposure in both program branches.`,
    `- Replay sample: ${report.metadata.schedule.replaySample}.`,
    `- Engine: ${report.metadata.engineVersion} (state schema ${report.metadata.stateSchemaVersion}; replay schema ${report.metadata.replaySchemaVersion})`,
